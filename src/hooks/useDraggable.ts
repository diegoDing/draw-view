...
export default function (){
    ...
    const dragleaveEvent=function( event:DragEvent ) {
        ...
        if ( event.target instanceof HTMLElement&&event.target.className == "canvas-content" ) {
    ...
    const mapEventsCallback=function (eventName:EventName,container:HTMLElement){
        map.forEach((value,key)=>{
            const events:Events|undefined=map.get(key)
            if (events){
                const data:UseDraggableEventData={
                    target:dragged,container,pageX,pageY
                }
